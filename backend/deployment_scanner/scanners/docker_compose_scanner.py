...
class DockerComposeScanner:
    ...
    def _generate_report(self):
        ...
        if isinstance(total_sec_issues, int) and total_sec_issues > 0:
            print(f"\n⚠️  PROBLEMY BEZPIECZEŃSTWA: {total_sec_issues}")
    ...
    def _print_service_report(self, service: ServiceInfo):
        ...
        if service.security_issues:
            print(f"\n⚠️  PROBLEMY BEZPIECZEŃSTWA ({len(service.security_issues)}):")
    ...
    def _generate_summary(self) -> Dict[str, Union[int, Dict[str, int]]]:
        ...
        # Policz problemy według severity
    ...
    def _analyze_security_issues(
        ...
        # Sprawdź niebezpieczne zmienne środowiskowe
        environment = config.get("environment", {})
        if isinstance(environment, list):
            # Konwertuj listę na dict
            env_dict = {}
        ...
        # Sprawdź niebezpieczne volume mounts
        volumes = config.get("volumes", [])
        for volume in volumes:
            volume_str = str(volume)
            # Sprawdź czy montuje wrażliwe ścieżki systemu
            sensitive_paths = [
                "/",
                "/root",
                "/etc",
